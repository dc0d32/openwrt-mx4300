#!/usr/bin/env bash
# Materialize the immutable upstream OpenWrt source for the non-NSS MX4300 build.
set -euo pipefail
...
if [ ! -d "$OPENWRT_DIR/.git" ]; then
  ...
  git clone --filter=blob:none --no-checkout "$VANILLA_REPOSITORY" "$OPENWRT_DIR"
  new_checkout=true
fi

actual_origin="$(git -C "$OPENWRT_DIR" remote get-url origin)"
[ "$actual_origin" = "$VANILLA_REPOSITORY" ] ||
...
git -C "$OPENWRT_DIR" fetch --depth=1 origin "$VANILLA_COMMIT"
git -C "$OPENWRT_DIR" checkout --detach "$VANILLA_COMMIT"
cp "$ROOT/vanilla-feeds.conf.lock" "$OPENWRT_DIR/feeds.conf"
install -D -m 0600 "$ROOT/files/etc/dropbear/authorized_keys" \
  "$OPENWRT_DIR/files/etc/dropbear/authorized_keys"

echo "Prepared upstream OpenWrt $VANILLA_RELEASE@$VANILLA_COMMIT in $OPENWRT_DIR"
