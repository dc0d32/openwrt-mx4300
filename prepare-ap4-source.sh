#!/usr/bin/env bash
# Materialize the exact official OpenWrt source and feeds used by AP-4.
set -euo pipefail
...
if [ ! -d "$OPENWRT_DIR/.git" ]; then
  ...
  git clone --filter=blob:none --no-checkout "$AP4_REPOSITORY" "$OPENWRT_DIR"
  new_checkout=true
fi

actual_origin="$(git -C "$OPENWRT_DIR" remote get-url origin)"
[ "$actual_origin" = "$AP4_REPOSITORY" ] ||
...
git -C "$OPENWRT_DIR" fetch --depth=1 origin "$AP4_COMMIT"
git -C "$OPENWRT_DIR" checkout --detach "$AP4_COMMIT"
cp "$ROOT/ap4-feeds.conf.lock" "$OPENWRT_DIR/feeds.conf"
install -D -m 0600 "$ROOT/files/etc/dropbear/authorized_keys" \
  "$OPENWRT_DIR/files/etc/dropbear/authorized_keys"

echo "Prepared openwrt/openwrt@$AP4_COMMIT in $OPENWRT_DIR"
